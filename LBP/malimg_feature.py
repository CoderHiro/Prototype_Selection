...
def build_vgg_model(num_classes):
    base_model = VGG16(weights="/home/cgh/prototype_selection/classify/model_10/new_Malimg_vgg16_all.h5", input_shape=(224, 224, 3), include_top=True, classes=num_classes)
    model = Model(inputs=base_model.input, outputs=base_model.get_layer('fc2').output)
    return model

def build_resnet50_model(num_classes):
    base_model = ResNet50(weights="/home/cgh/prototype_selection/classify/model_10/new_Malimg_resnet50_all.h5", input_shape=(224, 224, 3), include_top=True, classes=num_classes)
    model = Model(inputs=base_model.input, outputs=base_model.get_layer('avg_pool').output)
    return model
    
def build_incV3_model(num_classes):
    base_model = InceptionV3(weights="/home/cgh/prototype_selection/classify/model_10/new_Malimg_inceptionV3_all.h5", input_shape=(224, 224, 3), include_top=True, classes=num_classes)
    model = Model(inputs=base_model.input, outputs=base_model.get_layer('avg_pool').output)
    return model
...
if __name__ == "__main__":
    ...
    file_labels = {}
    label_path = '/home/cgh/prototype_selection/classify/data/BIG15_labels.csv'
    ...
    save_path = './malimg_output/{}_{}.txt'.format(dataset, model_type)
    index2name = './malimg_output/{}_selected_name_list.csv'.format(dataset)
    test_name = './malimg_output/{}_test_name_list.csv'.format(dataset)

    file_feature_list = []
    file_label_list = []
    file_index2name =[]

    # model = build_vgg_model(25)
    # model = build_resnet50_model(25)
    model = build_incV3_model(25)
