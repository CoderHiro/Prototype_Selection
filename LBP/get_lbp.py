...
class LBP:
  ...
  #获取图像的LBP等价模式特征
    ...
    def show_hist(self,img_array,im_bins,im_range):
        ...
        hist = cv2.normalize(hist,hist).flatten() # python2:cv2.normalize(hist,hist).flatten()
        return hist
...
def get_LBPH_from_image(image_path):
    image = image_path
    lbp_parser = LBP()
    image_array=lbp_parser.describe(image)
    resolve_uniform_array=lbp_parser.lbp_revolve_uniform(image_array)
    histogram = lbp_parser.show_revolve_uniform_hist(resolve_uniform_array)
    return histogram

if __name__ == '__main__':
    image = "/home/mayixuan/mal-visual/BIG-9-family/Gatak/01azqd4InC7m9JpocGv5.JPG"
    result = get_LBPH_from_image(image)
    print(result.tolist())
    # lbp=LBP()
